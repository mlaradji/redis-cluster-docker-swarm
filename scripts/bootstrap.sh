...
docker service create --network webnet --name redis-zero redis:5.0.9-alpine

echo "Starting services"
docker stack deploy -c scripts/docker-compose.yml cache

until [ "$(docker run --rm --network webnet mlaradji/redis-utils:$TAG \
...
old_master=$(docker run --rm --network webnet mlaradji/redis-utils:$TAG \
...
docker run --rm --network webnet mlaradji/redis-utils:$TAG \
